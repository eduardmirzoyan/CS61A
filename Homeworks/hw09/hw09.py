...
def roman_numerals(text):
    ...
    []
    """
    return re.findall("(\\b[IVXLCDM]+\\b)", text)
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search("((CS|cs)(\\s)?\\d+[ABCabc]?)", post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall("\\s([0-2]?[0-9]:[0-9][0-9][AM]*)", text)
...
def area_codes(text):
    ...
    []
    """
    return re.findall("\\(?(\\d{3})\\)?\\s?\\d{3}\\s?\\d{4}\\b", text)
