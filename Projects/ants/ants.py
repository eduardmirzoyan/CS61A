...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8            
            if place.ant is not None and place.ant.is_container and place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not start.is_hive:
            current_bee = random_bee(start.bees)
            if current_bee and self.min_range <= curr_range <= self.max_range:
                return current_bee
            curr_range += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for b in copy_bees:
            b.reduce_health(damage_to_deal)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if bee and self.chew_countdown == 0:
...
class TankAnt(ContainerAnt):
    """ TankAnt provides protection and deals damage to all bee in it's same place """
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not cls.is_true_queenAnt:
            cls.is_true_queenAnt = True
...
class Bee(Insect):
    ...
    damage = 1
    is_waterproof = True
    number_of_turns_slowed = 0

    has_been_scared = False
    number_of_retreats = 0
    # OVERRIDE CLASS ATTRIBUTES HERE
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            # Scared logic
            if self.number_of_retreats > 0: # if bee is scared
                #if scared AND slowed
                if self.number_of_turns_slowed > 0 and gamestate.time % 2 != 0:
                    pass
                else: # scared AND NOT slowed
                    if not self.place.entrance.is_hive:
                        self.move_to(self.place.entrance)
                    self.number_of_retreats -= 1
            else: # slow logic
                if self.number_of_turns_slowed > 0: # if bee is slowed
                    if gamestate.time % 2 == 0:
                        self.move_to(destination)
                else:
                    self.move_to(destination) # if no status effect, then move normally

            # decrement
            if self.number_of_turns_slowed > 0:
                self.number_of_turns_slowed -= 1
    ...
    def slow(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        self.number_of_turns_slowed += length
        # END Problem EC
    ...
    def scare(self, length):
        ...
        if not self.has_been_scared:
            self.number_of_retreats = length
