...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    #"*** YOUR CODE HERE ***"
    def is_paragraph_in_topic(paragraph):
        paragraph = lower(paragraph)
        paragraph = remove_punctuation(paragraph)
        all_words = split(paragraph)
        for i in range(len(topic)):
            if topic[i] in all_words:
                return True

        return False

    return is_paragraph_in_topic
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    return correct_counter / len(typed_words) * 100
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    min_valid_word = min(valid_words, key = lambda valid_word: diff_function(typed_word, valid_word, limit))

    if diff_function(typed_word, min_valid_word, limit) > limit:
        return typed_word
...
def feline_flips(start, goal, limit):
    ...
    #count = 0
    def feline_flips_helper(start, goal, new_limit):
        ...
        return add_one + feline_flips_helper(start[1:], goal[1:], new_limit - add_one)

    return feline_flips_helper(start, goal, limit)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    def minimum_mewtations_helper(start, goal, new_limit):
        if start == goal:
            return 0
        elif len(start) == 0:
            return 1 + minimum_mewtations(goal[0] + start, goal, new_limit - 1)
        elif len(goal) == 0:
            return 1 + minimum_mewtations(start[1:], goal, new_limit - 1)
        elif new_limit < 0:
            return limit + 1
        elif start[0] == goal[0]:
            return minimum_mewtations(start[1:], goal[1:], new_limit)
        else:
            add = 1 + minimum_mewtations(goal[0] + start, goal, new_limit - 1)
            remove = 1 + minimum_mewtations(start[1:], goal, new_limit - 1)
            substitute = 1 + minimum_mewtations(goal[0] + start[1:], goal, new_limit - 1)
            return min(add, remove, substitute)

    return minimum_mewtations_helper(start, goal, limit)
...
def time_per_word(words, times_per_player):
    ...
    for k in range(len(times_per_player)):
        total_time_differences += [[times_per_player[k][i+1] - times_per_player[k][i] for i in range(len(times_per_player[k])-1)]]    
    return match(words, total_time_differences)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for k in word_indices:
        ...
        for i in player_indices:
            if players_times[i][k] < players_times[min_time_player_index][k]: 
                min_time_player_index = i
        fastest_words_per_player[min_time_player_index] += [players_words[k]]
